// Inspired by:
// https://github.com/kentcdodds/kentcdodds.com/blob/main/app/utils/markdown.server.ts
...
async function getSitemapXml(request: Request, remixContext: EntryContext) {
  ...
  function getEntry({ route, lastmod, changefreq, priority }: TSitemapEntry) {
    return `
<url>
  ...
  const rawSitemapEntries = (
    await Promise.all(
      Object.entries(remixContext.routeModules).map(async ([id, mod]) => {
        ...
        const handle = mod.handle as THandle | undefined;
        ...
        const entry: TSitemapEntry = { route: removeTrailingSlash(path) };
        return entry;
      })
    )
  )
  ...
  const sitemapEntries: Array<TSitemapEntry> = [];
...
type THandle = {
  ...
  getSitemapEntries?: (
    ...
    | Promise<Array<TSitemapEntry | null> | null>
    | Array<TSitemapEntry | null>
    | null;
};
